...
function addCoverSlide(title, subtitle) {
  ...
  // 底部 meta
  slide.addText("2026-07-04 · SWISS STYLE", {
    x: 0.5,
    y: 6.9,
    w: 12.33,
    h: 0.4,
...
function addSectionSlide(partNum, title) {
  ...
  // 右上角页码（小字号）
  slide.addText(`${String(pageNum).padStart(2, "0")} / 63`, {
    x: 11.5,
    y: 0.3,
    w: 1.5,
    h: 0.3,
...
// 内容页（全宽，列表式）
function addContentSlide(num, title, content) {
  pageNum++;
  const slide = pptx.addSlide();
  slide.background = { color: COLORS.paper };
...
// 内容页（左文右图，截图占位右侧）
function addContentWithScreenshot(num, title, content, screenshotText) {
  pageNum++;
  const slide = pptx.addSlide();
  slide.background = { color: COLORS.paper };
  ...
  slide.addShape(pptx.ShapeType.rect, {
    ...
    line: { color: COLORS.grey3, dashType: "dash", width: 2 },
  });
...
// 封面
addCoverSlide("Dify 使用讲解", "v3.3 | 2026-07-04");
...
// 保存文件
pptx.writeFile({ fileName: "docs/pptx/Dify-使用讲解-v3.3-swiss.pptx" })
  .then(() => {
    console.log("✓ PPT 生成成功：docs/pptx/Dify-使用讲解-v3.3-swiss.pptx");
