...
2. [设计决策](#2-设计决策)
3. [数据库设计](#3-数据库设计)
4. [Feature 1: 模型调用权限控制](#4-feature-1-模型调用权限控制)
5. [Feature 2: 发布审批权限控制](#5-feature-2-发布审批权限控制)
6. [API 接口设计](#6-api-接口设计)
7. [前端页面设计](#7-前端页面设计)
8. [权限矩阵](#8-权限矩阵)
9. [边界规则与错误处理](#9-边界规则与错误处理)
10. [数据迁移与向后兼容](#10-数据迁移与向后兼容)
11. [审计日志](#11-审计日志)
12. [测试策略](#12-测试策略)
...
Feature 2（发布审批）不新增表、不修改现有表。仅通过 Service 层逻辑实现权限门控。
